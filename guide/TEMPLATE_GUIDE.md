...
## 8. 코드 최적화 원칙 (제작 시점부터 지킬 것)

파일이 길어져 Claude Code가 못 불러오는 문제는 사후 정리보다 제작 시점에 막는 게 우선이다.

- **새 섹션 작성 시 `<style>` 블록에 새 전역 CSS 규칙을 추가하지 않는다.** 위 §4에 정의된 컴포넌트 클래스(callout, compare, flow, stat-row 등)만 조합해서 사용한다.
- 표현하려는 내용에 맞는 컴포넌트가 §4에 없다면, 임의로 새 CSS를 만들지 말고 먼저 사용자에게 확인한다.
- 위치 지정(`style="left:.."` 같은 좌표성 인라인 스타일)은 Positioning Map·Bar Chart처럼 원래 인라인으로 쓰도록 설계된 컴포넌트에서만 사용한다. 그 외 요소에 인라인 스타일로 디자인을 우회하지 않는다.
- 섹션 내용을 다시 쓸 때 이전 버전 내용을 주석(`<!-- -->`)으로 남겨두지 않는다 — 필요하면 `archive/`에 이미 버전이 남아있으므로 파일 안에는 최종본만 유지한다.
